...
class ExecutionResultRef(BaseRef):
    ...
    def get_input(
        ...
        (
            ...
            self._backend_info,
            self._input_program,
        ) = self._get_execute_results(ResultVersions.DEFAULT, scope=scope)
    ...
    def download_result(
        ...
        (
            ...
            self._backend_info,
            self._input_program,
        ) = self._get_execute_results(version=version, scope=scope)
        self._result_version = version
        return copy(self._result)

    @merge_scope_from_context
    def download_backend_info(
        self, scope: ScopeFilterEnum = ScopeFilterEnum.USER
    ) -> BackendInfo:
    ...
    def _get_execute_results(
        self, version: ResultVersions, scope: ScopeFilterEnum = ScopeFilterEnum.USER
        ...
        match self.result_type:
            case ResultType.PYTKET:
                ...
                return _fetch_pytket_execution_result(self, scope=scope)
            case ResultType.QSYS:
                return _fetch_qsys_execution_result(self, version, scope=scope)
            case _:
                assert_never(self.result_type)

    @merge_scope_from_context
    def download_h2_qsysresult(
        self, scope: ScopeFilterEnum = ScopeFilterEnum.USER
    ) -> QsysResult:
        """Get the QSysResult for a hugr-qir execution result on a H2-generation system.

        This is only valid for results obtained for executions of QIR generated by hugr-qir from guppylang.

        NB! for this function to work you need to install hugr-qir >= 0.3.0

        Examples:
            >>> import qnexus as qnx
            >>> execute_results = qnx.jobs.results(hugr_qir_execute_job_ref)
        ...
        result = self.download_result(scope=scope)
