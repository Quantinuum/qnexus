...
class ExecutionResultRef(BaseRef):
    ...
    def download_h2_qsysresult(self) -> QsysResult:
        """Get the QSysResult for a hugr-qir execution result.
        ...
        try:
            from hugr_qir.h_series_helpers.results import (
                ...
                backendresult_to_qsysresult_with_qir,
            )
        except ImportError:
            raise ValueError(
                "Please install hugr-qir via 'pip install hugr-qir==0.3.0rc1'"
            )
        ...
        try:
            ...
            try:
                qir = self.get_input().download_qir()
                return backendresult_to_qsysresult_with_qir(self.download_result(), qir)
            except AttributeError as e:
                raise AttributeError(
                    "Please check that the input program has a downloadable QIR that was generated with hugr-qir"
                ) from e
